...
<header><div class="brand">MODÜLER MASRAF v18.9</div>${u?`<nav><a href="/">Ana Sayfa</a><a href="/expenses/new">Masraf Ekle</a><a href="/expenses">Masraflarım</a>${isManager(u)?'<a href="/approvals">Onaylar</a><a href="/admin">Yönetim</a>':''}<a href="/logout">Çıkış</a></nav>`:''}</header>
<main>${body}</main>
...
app.get('/health',(req,res)=>res.json({ok:true,version:'18.9.1'}));
app.get('/api/version',(req,res)=>res.json({version:'18.9.1',name:'v18.9 rollback'}));
app.get('/api/ocr-status',(req,res)=>res.json({provider:process.env.OCR_PROVIDER||'google',googleVision:!!process.env.GOOGLE_VISION_API_KEY,openai:false,version:'18.9.1'}));
...
function cleanNum(s){ if(!s) return ''; let x=String(s).replace(/[^0-9,\.]/g,''); if(x.includes(',') && x.includes('.')) x=x.replace(/\./g,'').replace(',','.'); else x=x.replace(',','.'); const n=Number(x); return Number.isFinite(n)?n.toFixed(2):''; }
function parseDateTR(text){ const m=String(text).match(/\b(\d{1,2})[\.\/-](\d{1,2})[\.\/-](\d{2,4})\b/); if(!m) return ''; let y=m[3].length===2?'20'+m[3]:m[3]; const yyyy=Number(y); if(yyyy<2000 || yyyy>new Date().getFullYear()+1) return ''; return `${String(yyyy).padStart(4,'0')}-${String(m[2]).padStart(2,'0')}-${String(m[1]).padStart(2,'0')}`; }
function parseReceipt(text){
  const lines=String(text||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean);
  const joined=lines.join('\n');
  let companyName='';
  for(const l of lines.slice(0,8)){ if(!/^(tarih|fis|fiş|belge|saat|z no|ekü|kdv|toplam|ara toplam)/i.test(l) && l.length>3){ companyName=l.replace(/\s*%\d+\s*$/,''); break; } }
  const documentDate=parseDateTR(joined);
  const receiptMatch=joined.match(/(?:fiş|fis)\s*(?:no|numarası|numarasi)?\s*[:\-]?\s*([A-Z0-9\-\/]{3,})/i);
  const docMatch=joined.match(/(?:belge|fatura)\s*(?:no|numarası|numarasi)?\s*[:\-]?\s*([A-Z0-9\-\/]{3,})/i);
  const moneyMatches=[...joined.matchAll(/(?:₺|tl)?\s*(\d{1,3}(?:[\. ]\d{3})*[,.]\d{2}|\d+[,.]\d{2})\s*(?:tl|₺)?/gi)].map(m=>cleanNum(m[1])).filter(Boolean).map(Number);
  const totalAmount=moneyMatches.length?Math.max(...moneyMatches).toFixed(2):'';
  let vatAmount='', taxBase='';
  const vatLine=lines.find(l=>/kdv/i.test(l) && /\d+[,.]\d{2}/.test(l));
  if(vatLine){ const ms=[...vatLine.matchAll(/(\d{1,3}(?:[\. ]\d{3})*[,.]\d{2}|\d+[,.]\d{2})/g)].map(m=>Number(cleanNum(m[1]))); if(ms.length) vatAmount=ms[ms.length-1].toFixed(2); }
  if(totalAmount && !vatAmount && /%\s*20|KDV.*20/i.test(joined)){ vatAmount=(Number(totalAmount)*20/120).toFixed(2); }
  if(totalAmount && vatAmount){ taxBase=(Number(totalAmount)-Number(vatAmount)).toFixed(2); }
  return { companyName, documentDate, documentNo: docMatch?docMatch[1]:'', receiptNo: receiptMatch?receiptMatch[1]:'', taxBase, vatAmount, totalAmount };
}
app.post('/api/ocr',requireAuth,upload.single('receipt'),async(req,res)=>{
  try{
    ...
    res.json({ok:true,provider:'google',version:'18.9.1',textLength:text.length,mimeType:req.file.mimetype,fileSize:req.file.size,parsed,text:text.slice(0,2500)});
...
app.listen(PORT,()=>console.log(`Modüler Masraf v18.9.1 çalışıyor: ${PORT}`));
